...
def controller(out, className, tableComment, paramName, fields) {
    ...
    out.println"    public ResponseEntity<?> get${className}List(@ApiParam(name = \"pageNum\", value = \"当前页码\", required = true) @RequestParam(name = \"pageNum\") Integer pageNum,"
    out.println"                                                @ApiParam(name = \"pageSize\", value = \"页大小\", required = true) @RequestParam(name = \"pageSize\") Integer pageSize) {"
    out.println"        ${className} ${paramName} = new ${className}();"
